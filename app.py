...
def _select_active_policy(data, student_email):
    ...
    We normalize emails to lowercase so that assignments like
    'Student@School.org' and 'student@school.org' match consistently.

    A single email (or group) may be assigned to multiple policies. In that
    case, we consider *all* of those policies as applicable and then choose
    the highest-priority one that is currently active.
    """
    data = ensure_keys(data or {})
    policies = data.get("policies", {}) or {}
    assigns = data.get("policy_assignments", {}) or {}

    # Normalize user assignment keys to lowercase and coerce values
    # into a list of policy ids.
    raw_user_map = assigns.get("users", {}) or {}
    user_map = {}
    for raw_key, raw_val in raw_user_map.items():
        key = (raw_key or "").strip().lower()
        if not key:
            continue
        if isinstance(raw_val, list):
            vals = [v for v in raw_val if v]
        elif raw_val:
            vals = [raw_val]
        else:
            vals = []
    ...
    # Groups may also (optionally) be mapped to a single policy id or a list.
    raw_group_map = assigns.get("groups", {}) or {}
    group_map = {}
    for raw_key, raw_val in raw_group_map.items():
    ...
    if student_email and student_email in user_map:
        for pid in user_map.get(student_email, []):
            if pid:
                applicable_ids.add(pid)

    # Class/group-based policies: if student email is listed as a member of a class,
    # and that class has a group policy assignment, include it.
    for cid, cls in (data.get("classes") or {}).items():
        try:
            students = cls.get("students") or []
        except Exception:
            students = []
        # Normalize stored student emails to lowercase for comparison
        lowered = [ (s or "").strip().lower() for s in students ]
        if student_email and student_email in lowered:
            for pid in group_map.get(cid, []):
                if pid:
                    applicable_ids.add(pid)

    # Fall back to default policy if nothing else applies
    if not applicable_ids and default_id:
        applicable_ids.add(default_id)

    # Filter down to those that are currently active by schedule
    active = []
    ...
    # Highest priority wins (larger number = higher priority; 0 is the lowest)
    active.sort(key=lambda p: p.get("priority", 0), reverse=True)
    return active[0]
...
def api_policy():
    ...
    # Class config (single default class for now)
    ...
    if student:
        ...
        pending = pending_all.get(student, []) or []
        if student in pending_all:
    ...
    # Shape the active policy payload for the extension (include blocked_categories)
    ap = None
    if active_policy:
        ap = {
...
def api_policy_assignments():
    ...
    if request.method == "GET":
        return jsonify({
            "ok": True,
            "policy_assignments": d.get("policy_assignments", {}),
            "default_policy_id": d.get("default_policy_id"),
        })

    body = request.json or {}
    assigns = d.get("policy_assignments", {}) or {}
    assigns.setdefault("users", {})
    assigns.setdefault("groups", {})

    # Optional: when the admin UI is saving a single policy, it can tell us
    # which policy id is currently being edited. This lets us treat the
    # "users" patch as "add/remove this one policy" instead of overwriting
    # everything for that email.
    current_policy_id = (body.get("current_policy_id") or "").strip() or None

    for scope in ("users", "groups"):
        patch = body.get(scope)
        if not isinstance(patch, dict):
            continue

        for raw_key, pid in patch.items():
            key = (raw_key or "").strip()
            if not key:
                continue

            # Normalize user emails to lowercase when storing
            if scope == "users":
                key = key.lower()

            # When we know which policy is being edited, treat values as
            # "add/remove this policy" and store a list of policies.
            if current_policy_id and scope == "users":
                cur_val = assigns[scope].get(key)

                # Coerce existing value into a list
                if isinstance(cur_val, list):
                    cur_list = [v for v in cur_val if v]
                elif cur_val:
                    cur_list = [cur_val]
                else:
                    cur_list = []
                ...
                if pid is None:
                    # Remove this policy from the list
                    cur_list = [v for v in cur_list if v != current_policy_id]
                    if cur_list:
                        assigns[scope][key] = cur_list
                    else:
                        assigns[scope].pop(key, None)
                else:
                    # Add this policy to the list if not already present
                    if current_policy_id not in cur_list:
                        cur_list.append(current_policy_id)
                    assigns[scope][key] = cur_list
            else:
                # Backwards-compatible behaviour: single policy per email/group.
                if pid is None:
                    assigns[scope].pop(key, None)
                else:
                    assigns[scope][key] = pid
...
def api_notify():
    ...
    d = ensure_keys(load_data())
    target = student if student else "*"

    d.setdefault("pending_commands", {}).setdefault(target, []).append({
        "type": "notify",
        "title": title,
        "message": message,
    })
